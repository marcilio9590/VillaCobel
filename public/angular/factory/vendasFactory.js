...
myApp.factory('vendasFactory', function () {
	...
	function verificarItensProjeto(antigo, novo) {
		var itensUpdate = [];
		antigo.itensProjeto.forEach(function (oldItem) {
			var item = {};
			novo.itensUpdate.forEach(function (newItem) {
				if (oldItem.idItem === newItem.iditem) {
					item = {
						"qtd": Number(newItem.qtd) > Number(oldItem.qtd) ? Number(newItem.qtd) - Number(oldItem.qtd) : Number(newItem.qtd),
						"iditem": newItem.iditem,
						"tipo": newItem.tipo
					};
				}
			});
			if(itensUpdate.length === 0 && item.iditem){
				itensUpdate.push(item);					
			}else{
				if(itensUpdate.indexOf(item) === -1){
					itensUpdate.push(item);
				}
			}
		});
	...
	var exports = {
		...
		verificarItensProjeto: verificarItensProjeto
	};
