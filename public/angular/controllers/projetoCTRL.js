myApp.controller("projetoCTRL", function ($scope, $http, projetoFactory, produtosFactory, projetoService, $timeout, produtoService) {
	var vm = $scope;
	vm.step1 = true;
	vm.step2 = false;
	vm.mostra = false;
	...
	vm.editarProjeto = function (projeto) {
		...
		vm.oldProjetoEditar = angular.copy(projeto);
		$("#myModal").modal();
	}
	...
	vm.removerItem = function (item) {
		if (!angular.isUndefined(item.codigo)) {
			for (var i = 0; i < vm.listEstoque.length; i++) {
				if (vm.listEstoque[i].codigo == item.codigo) {
					if (item.tipo == "quantidade") {
						vm.listEstoque[i].quantidade += item.novaQtd;
						vm.estoqueVazio = false;
					} else if (item.tipo == "metro") {
						vm.listEstoque[i].metro += item.novoMetro;
						vm.estoqueVazio = false;
					}
				}
			}
			for (var i = 0; i < vm.adicionadosProjeto.length; i++) {
				if (vm.adicionadosProjeto[i].codigo === item.codigo) {
	...
	/**
	 * list projects
	 * return list of projects and items.
	 */
	vm.listarProjetos = function () {
		return projetoService.listProjects().then(function successCallback(response) {
	...
	vm.buscarItens = function (codigo, index) {
		produtoService.searchProducts(codigo).then(function successCallback(response) {
	...
	vm.salvarEdicao = function () {
		...
		projetoService.editarProjeto(proj).then(function (response) {
			var updateEstoque = projetoFactory.verificarItensProjeto(vm.oldProjetoEditar, proj);
